...
describe('Zip search module', () => {
  it('Should get a valid response when passing a valid zip code', (done) => {
    const zipCode = index.getZipCode('31652130', true);  
  ...
  it('Should get a valid response when returning promise', () => {
    makeDefaultNock();
    return expect(index.getZipCode('31652130')).to.eventually.deep.equal(DEFAULT_RESPONSE);
  });
  
  it('Should work when passing a zipCode with dash', () => {
    makeDefaultNock();
    return expect(index.getZipCode('31652-130')).to.eventually.deep.equal(DEFAULT_RESPONSE);
  });
  ...
  it('Should return an error message when passing wrong parameter', () => {
    const result = index.getZipCode('wrong', true);
  ...
  it('Should return an error message when passing wrong parameter (async)', () => {
    const result = index.getZipCode('wrong').catch(err => err.message);
  ...
  it('Should handle wrong zip codes', () => {
    ...
    const result = index.getZipCode('123456ab').catch(err => err.statusCode);
